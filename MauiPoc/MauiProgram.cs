...
public static class MauiProgram
{
    ...
    public static MauiApp CreateMauiApp()
    {
        using var stream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("MauiPoc.appsettings.json") ?? throw new Exception("appsettings.json not found");
        ...
        using var androidStream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("MauiPoc.appsettings.Android.json") ?? throw new Exception("appsettings.Android.json not found");
        configBuilder = configBuilder.AddJsonStream(androidStream);
#endif
#if IOS
        using var iosStream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("MauiPoc.appsettings.Ios.json") ?? throw new Exception("appsettings.Ios.json not found");
        configBuilder = configBuilder.AddJsonStream(iosStream);
#endif
#if !DEBUG
        using var releaseStream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("MauiPoc.appsettings.Release.json") ?? throw new Exception("appsettings.Release.json not found");
        configBuilder = configBuilder.AddJsonStream(releaseStream);
#endif
#if DEBUG
        using var debugStream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("MauiPoc.appsettings.Debug.json") ?? throw new Exception("appsettings.Debug.json not found");
